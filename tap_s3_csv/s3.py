...
def log_backoff_attempt(details):
    ...
    :return:
    """
    internal_logger.info("Error detected communicating with Amazon, triggering backoff: %d try", details.get("tries"))
...
def setup_aws_client(config: Dict) -> None:
    ...
    """
    internal_logger.info("Attempting to create AWS session")
...
def get_sampled_schema_for_table(config: Dict, table_spec: Dict) -> Dict:
    ...
    """
    user_logger.info("Sampling records to determine table schema.")
    ...
    for row in iterator:
        if (current_row % sample_rate) == 0:
            ...
            if (sampled_row_count % 200) == 0:
                internal_logger.info("Sampled %s rows from %s", sampled_row_count, s3_path)
            yield row
...
def sample_files(
    ...
    """
    internal_logger.info("Sampling files (max files: %s)", max_files)
    for s3_file in more_itertools.tail(max_files, s3_files):
        internal_logger.info("Sampling %s (max records: %s, sample rate: %s)", s3_file["key"], max_records, sample_rate)
...
def get_input_files_for_table(config: Dict, table_spec: Dict, modified_since: str = None) -> Generator:
    ...
    for s3_object in sorted(
        list_files_in_bucket(bucket, prefix, aws_endpoint_url=config.get("aws_endpoint_url")),
        ...
        if s3_object["Size"] == 0:
            internal_logger.info('Skipping matched file "%s" as it is empty', key)
        ...
        if matcher.search(key):
            ...
            if modified_since is None or modified_since < last_modified:
                internal_logger.info('Will download key "%s" as it was last modified %s', key, last_modified)
        ...
        if (unmatched_files_count + matched_files_count) % max_files_before_log == 0:
            ...
            if (unmatched_files_count / (matched_files_count + unmatched_files_count)) > 0.5:
                ...
                internal_logger.info(
                    "Found %s matching files and %s non-matching files", matched_files_count, unmatched_files_count
                )
...
def list_files_in_bucket(bucket: str, search_prefix: str = None, aws_endpoint_url: Optional[str] = None) -> Generator:
    ...
    if s3_object_count > 0:
        internal_logger.info("Found %s files.", s3_object_count)
    else:
        internal_logger.warning('Found no files for bucket "%s" that match prefix "%s"', bucket, search_prefix)
